...
| `morse`      | `pin`, `message`, `unit?`, `loop?`                            | Blink the pin as the Morse code of `message`. `unit` is seconds per dot (dash 3×, gaps standard); default 0.2. `loop: true` repeats until stopped; otherwise plays once then leaves the pin LOW. Unknown characters are skipped. |
...
`blink`, `morse`, and `servo` (with `duration`) are _continuous_: they keep
running on a background tick after the cue fires. Everything else is
instantaneous.
